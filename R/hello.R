  ...
  if (distribution == "weibull"){
    ...
    if (minshape >= 1){
      ...
      if (plots == TRUE){
        ...
        curve(dweibull(x, seq(minshape, maxshape, by=(maxshape-1)/steps1)[which.max(abs(corvec))]), col = "green", add = TRUE)
      ...
      if (plots == TRUE){
        hist(x, freq = FALSE, breaks = brks)
        curve(dweibull(x, seq(minshape, maxshape, by=(1-minshape)/steps2)[which.max(abs(corvec))]), col = "green", add = TRUE)
        lines(density(x, bw = bandw, from = 0), col = "red")
        legend("topleft", c("True Weibull density", "Estimated density"), lty = 2, col = c("green", "red"))
      }
      cat("The highest absolute correlation coefficient is", max(abs(corvec)), "\n")
